...
exports.run = async (client, message, args) => {
    ...
    if(!username){
        return message.channel.send({embed: {
            color: 16733013,
            title: ":x: Error! :x:",
            description: "I can not fire a user if you don't give me a username!",
        }});
    }
    let id;
    try {
    ...
    if(Number(process.env.maximumRank) <= rankInGroup){
        return message.channel.send({embed: {
            color: 16733013,
            description: "I can not fire this user.",
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL()
            }
    ...
    try {
        ...
        return message.channel.send({embed: {
            color: 16733013,
            description: `An error occured, user was not fired.`,
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL()
            }
