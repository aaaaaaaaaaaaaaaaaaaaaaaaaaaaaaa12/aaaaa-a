...
exports.run = async (client, message, args) => {
    ...
    if(Number(process.env.maximumRank) <= rankInGroup){
        return message.channel.send({embed: {
            color: 16733013,
            title: ":x: Error! :x:",
            description: "I can not demote that user.",
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL()
            }
    ...
    try {
        ...
        console.log(chalk.red('An error occured while using the demote command ' + err));
        return message.channel.send({embed: {
            color: 16733013,
            title: `:x: Error! :x:`,
            description: `An error occured, user was not demoted.`,
            }
         });                           
    }
    ...
    message.channel.send({embed: {
        ...
        description: `Demoted ${username} to ${demoteResponse.newRole.name} (${demoteResponse.newRole.rank})`,
        }
    });
