...
exports.run = async (client, message, args) => {
    if(!message.member.roles.cache.some(role =>["Ranker"].includes(role.name))){
    ...
    if(!username){
        return message.channel.send({embed: {
            color: 16733013,
            title: ":x: Error! :x:",
            description: "I can not give trainee rank to a user if you don't give me a username!",
        }});
    }
    let id;
    try {
    ...
    if(Number(process.env.maximumRank) <= rankInGroup){
        return message.channel.send({embed: {
            color: 16733013,
            description: "I can not give trainee rank to this user.",
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL()
            }
    ...
    let fireResponse;
    try {
        fireResponse = await roblox.setRank(Number(process.env.groupId), id, 5);
    } catch (err) {
        console.log(chalk.red('An error occured when running the trainee command: ' + err));
        return message.channel.send({embed: {
            color: 16733013,
            description: `An error occured, user was not given trainee.`,
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL()
            }
    ...
    message.channel.send({embed: {
        ...
        description: `Successfully given trainee to ${username}!`,
